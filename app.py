...
Provide a brief, clear summary in 2-3 sentences. Format as clean HTML paragraphs using <p> tags only."""
            else:
                # Original detailed prompt
                prompt = f"""
                ...
                - DO NOT include any tables, lists, or follow-up questions
                - DO NOT use <ul>, <ol>, <li>, <table>, or similar tags
                - Focus only on summarizing the findings in paragraph form
                ...
                - Format as clean HTML paragraphs only
                """
            
            # Use faster model and reduced tokens
            model = "gpt-3.5-turbo" if PerformanceConfig.USE_FASTER_MODEL else "gpt-4"
            max_tokens = 300 if PerformanceConfig.REDUCE_AI_CONTEXT else 1000
            ...
            # Clean up the response - ensure it's properly formatted HTML
            if not ai_response.startswith('<p>'):
                # Convert plain text to HTML paragraphs
                paragraphs = ai_response.split('\n\n')
                ai_response = ''.join([f'<p>{p.strip()}</p>' for p in paragraphs if p.strip()])
            
            # Remove any unwanted elements
            import re
            # Remove any lists or follow-up questions
            ai_response = re.sub(r'<ul>.*?</ul>', '', ai_response, flags=re.DOTALL)
            ai_response = re.sub(r'<ol>.*?</ol>', '', ai_response, flags=re.DOTALL)
            ai_response = re.sub(r'<li>.*?</li>', '', ai_response, flags=re.DOTALL)
            
            # Remove follow-up question patterns
            ai_response = re.sub(r'<p>.*?follow.*?up.*?questions?.*?</p>', '', ai_response, flags=re.IGNORECASE)
            ai_response = re.sub(r'<p>.*?questions?.*?you.*?might.*?ask.*?</p>', '', ai_response, flags=re.IGNORECASE)
            
            # Check if response might be truncated
            if response.choices[0].finish_reason == 'length':
                ai_response += "<p><em>[Response was truncated due to length. Please ask a follow-up question for more details.]</em></p>"
            
            return ai_response
            
        except Exception as e:
            logger.error(f"Error enhancing response: {str(e)}")
            return f"<p>Found {query_result['row_count']} results for your query.</p>"
